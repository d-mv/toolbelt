import { concat, isEmpty, path, split, toLower } from 'ramda';
...
export function safeObject(obj: unknown, options?: Partial<ConfigToSafeOptions>): Record<string, unknown> {
  ...
  for (const key in obj as object) {
    ...
    if (value !== undefined && value !== null) {
      if (Array.isArray(value)) result[key] = value.map(v => safeObject(v, options));
        ...
        if (safeKeys.includes(key)) result[key] = value;
          ...
          if (isUnsafe) result[key] = isEmpty(value) ? '[MISSING]' : '[PRESENT]';
