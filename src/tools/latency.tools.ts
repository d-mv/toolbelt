...
export function perfMeasure(tag: string, startTagOrOptions?: string | Partial<PerfMeasureOptions>, endTag?: string) {
  let start = `${tag}.start`;
  let end = `${tag}.end`;
  let fractions = 2;
  ...
  const p = performance.measure(tag, start ?? `${tag}.start`, end ?? `${tag}.end`);

  logger.metrics(`${tag};latency:${Math.round(p.duration * fractions) / fractions}`);
}
