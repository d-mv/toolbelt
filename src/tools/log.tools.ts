...
class EventsClass {
	...
	#subscribeToNodeJsEvents() {
		...
		this.eventEmitter?.on(
			LogEventTypes.METRICS,
			function logInfoPrinter(message: string) {
		...
		this.eventEmitter?.on(
			LogEventTypes.WARN,
			function logWarnPrinter(message: string) {
		...
		this.eventEmitter?.on(
			LogEventTypes.DIR,
			function logDirPrinter(data: unknown) {
		...
		this.eventEmitter?.on(
			LogEventTypes.ERR,
			function logErrorPrint(payload: { error: Error; message: string }) {
		...
		this.eventEmitter?.on(
			LogEventTypes.WRITE,
			function logWritePrinter({
				message,
				eol = false,
			}: { message: string; eol?: boolean }) {
...
function write(message: string, eol = false) {
	if (!Events.isWeb)
		Events.send({ type: LogEventTypes.WRITE, message: { message, eol } });
}
